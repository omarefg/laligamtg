...
import { Nullable, User, UserMetadata } from '@/types';

export default function PlayerPage() {
  const params = useParams();
  const [player, setPlayer] = useState<Nullable<User>>(null);
              ...
              <div key={index} className="bg-mtg-gray rounded-lg p-4">
                ...
                {round.tables.map((table, tableIndex) => {
                  const playerPosition =
                    table.winner[0] === player.id ? '1º lugar' :
                      table.second[0] === player.id ? '2º lugar' :
                        table.third[0] === player.id ? '3º lugar' : '4º lugar';
